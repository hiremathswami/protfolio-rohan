...
export class AdiAssistant {
  ...
  processFallbackQuery(query) {
    ...
    if (q.includes('project') || q.includes('portfolio') || q.includes('built') || q.includes('work') || q.includes('showcase')) {
      let text = `${p.name} has built and deployed 5 featured software projects:\n\n`;
    ...
    if (techMatch) {
      ...
      let text = `Yes, ${p.name} has verified experience with ${techMatch}.\n\n`;
      if (matchingProjects.length > 0) {
        text += `Projects utilizing ${techMatch}:\n` +
          matchingProjects.map(m => `• ${m.name}: ${m.tagline}`).join('\n');
      } else {
        text += `${techMatch} is documented in ${p.name}’s verified technical toolbox.`;
      }
    ...
    if (q.includes('certif') || q.includes('credential') || q.includes('course') || q.includes('coursera') || q.includes('datacamp') || q.includes('aws') || q.includes('gcp')) {
      let text = `${p.name} holds 11 verified professional certifications and course achievements:\n\n` +
    ...
    if (q.includes('contact') || q.includes('email') || q.includes('linkedin') || q.includes('github') || q.includes('resume') || q.includes('hire') || q.includes('reach')) {
      return {
        text: `You can reach out to ${p.name} directly through any of the following verified channels:\n\n` +
          `• Email: ${contact.email}\n` +
    ...
    if (q.includes('available') || q.includes('open to work') || q.includes('internship') || q.includes('full-time') || q.includes('freelance')) {
      return {
        text: `${p.name}’s current verified availability status:\n` +
          `"${p.availability}"\n\n` +
    ...
    return {
      text: `That detail is not currently listed in ${p.name}’s portfolio. You can contact ${p.name} directly for the most accurate answer.`,
